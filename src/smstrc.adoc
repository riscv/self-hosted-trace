...
[[smstrc]]
== "Smstrce1p0/Smstrcn1p0" Self-hosted Trace ISA Extension, Version 1.0

Self-hosted trace describes the use of hardware trace capabilities by software running on the target.  The Smstrc* extensions provide an ISA interface to the RISC-V trace encoder.  'Sm' and 'Ss' imply M-mode and S-mode extensions, respectively, while 'strc' refers to self-hosted trace.  The following 'e' vs 'n' selects the encoding standard; 'e' implies the standard defined in the Efficient Trace for RISC-V v2.0 specification, while 'n' implies the standard defined in the RISC-V N-trace v1.0 specification.  Finally, '1p0' implies self-hosted trace version 1.0 for each encoding scheme.

Smstrc*/Ssstrc* utilize an asynchronous memory buffer (AMB) as the trace sink.  AMB is defined by the Ssamb extension, upon which Smstrc*/Ssstrc* depend.

The machine-level extensions, Smstrc*, encompasses all newly added Control Status Registers (CSRs), instructions, and behavior modifications for a hart across all privilege levels. The corresponding supervisor-level extensions, Ssstrc*, are essentially identical to the corresponding Smstrc* extension, except that it excludes machine-level CSRs and behaviors not intended to be directly accessible at the supervisor level.
...
The EN, EMPTY, IT, CEN, STOF, SMOD, SMAX, TSEN, U, S, and M fields are required, including all values for multi-bit fields, while others are optional.  WARL-discovery can be used to discover support for optional fields.  Unimplemented fields must be hardcoded to 0.

NOTE: _Most of the fields listed above correspond to equivalent fields in the `trTeControl` and `trTeInstFeatures` registers from the RISC-V Trace Control Interface Specification.  See <<strctci>> for details on self-hosted trace ramifications on the Trace Control Interface._
...
NOTE: _The Sdtrig extension specifies that `mcontext` is required when `hcontext` is implemented._
...
If the H extension is implemented, the `vsstrcctl` register is a 64-bit WARL register that is VS-mode’s version of supervisor register `sctrctl`. When V=1, `vsstrcctl` substitutes for the usual `sstrcctl`, so instructions that normally read or modify `sstrcctl` actually access `vsstrcctl` instead.
...
When Smstateen is implemented, the `mstateen0`.STRC bit controls access to STRC register state from privilege modes less privileged than M-mode.  When `mstateen0`.STRC=1, supervisor level accesses to STRC register state behave as described in <<CSRs>> above.  When `mstateen0`.STRC=0 and the privilege mode is less privileged than M-mode, attempts to access `sstrc*` or `vsstrcctl` raise an illegal-instruction exception, and `henvcfg`.STRCV is read-only 0.
...
When `mstateen0`.STRC=1 and `hstateen0`.STRC=1, VS-mode accesses to supervisor STRC state behave as described in <<CSRs>> above.  When `mstateen0`.STRC=1 and `hstateen0`.STRC=0, VS-mode accesses to supervisor STRC state raise a virtual-instruction exception.

Access to `scontext` and `hcontext` is governed not by `__x__stateen0`.STRC but by `__x__stateen0`[57].  Software should set `__x__stateen0`[57]=1 when `__x__stateen0`.STRC=1.
...
* When implicit returns are enabled (`mstrcctl`.EIR=1), the trace encoder must guarantee that only well-behaved returns are implicit; the decoder should never be redirected to the wrong return target.  The maximum return stack size is 64 entries.
...
For Smstrce1p0/Ssstrce1p0, when `mtrcctl`.TSEN=1, bits _time_width_p_-1:16 of the timestamp value are reported in format 3 packets, while bits 15:0 of the timestamp value are reported in the packet encapsulation.  This implies that the encapsulation header.extend bit is set to 1, and the timestamp width in bytes, T, is 2.  See section 2.1.3 of the Unformatted Trace & Diagnostic Data Packet Encapsulation for RISC-V v1.0 Specification for details on encapsulation timestamping.
...
Some implementations may opt to include only a subset of the `time` CSR value when the full trace timestamp is reported, to reduce both trace packet size and the hardware required to support it.  It is recommended that implementations include enough lower bits of `time` to ensure the timer does not wrap (i.e., repeat the same time value) for at least 24 hours.

NOTE: _While instruction traces generally capture fractions of a second of software execution, the duration of trace enabling may be much longer.  Consider a scenario where trace is written to a circular buffer, such that the buffer may wrap multiple times and only the last buffer-size bytes are collected and processed.  In such usages software tools cannot reliably detect if the full trace timestamp wrapped while the trace was running, or how many times.  The recommended timestamp duration above is intended to ensure that the full trace timestamp width is sufficient to support the vast majority of even circular-buffer usages._
...
When context reporting is enabled (`mstrcctl`.CEN=1), the combination of `hcontext` and `scontext` must be reported.  For N-trace, these are reported in separate messages/packets.  For E-trace, a single context value is reported, using the format given below.
...
|_blocks_p_ | Implementation-defined |  Number of times *iretire*, *itype* etc. are replicated
...
|_call_counter_size_p_ | Implementation-defined |  Number of bits in the nested call counter is
...
|_ecause_choice_p_ | Implementation-defined |  Number of bits of exception cause to match using
multiple choice
|_f0s_width_p_ | Implementation-defined |  Width of the *subformat* field in format 0 _te_inst_ packets.
|_filter_context_p_ | Implementation-defined | Filtering on context supported when 1
|_filter_time_p_ | Implementation-defined | Filtering on time supported when 1
|_filter_excint_p_ | Implementation-defined |  Filtering on exception cause or interrupt
...
1
|_filter_tval_p_ | Implementation-defined | Filtering on trap value supported when 1
(provided _filter_excint_p_ is non-zero)
|_iaddress_lsb_p_ | 1 if C extension implemented |  LSB of instruction *address* bus to trace. 1 is
compressed instructions are supported, 2 otherwise.
|_iaddress_width_p_ | MXLEN |  Width of instruction *address* bus. This is the
same as _DXLEN_
|_iretire_width_p_ | Implementation-defined |  Width of the *iretire* bus
|_ilastsize_width_p_ | Implementation-defined |  Width of the *ilastsize* bus
|_itype_width_p_ | Implementation-defined |  Width of the *itype* bus
...
|_retires_p_ | Implementation-defined |  Maximum number of instructions that can be retired per
block
|_return_stack_size_p_ | ≤6 |  Number of entries in the return address stack
...
|_sijump_p_ | Implementation-defined | *sijump* is used to identify sequentially
inferable jumps
|_impdef_width_p_ | Implementation-defined |  Width of *implementation-defined input* bus
|===

===== Required Support Packet

There are 4 parameters whose values may vary across Smstrce*/Ssstrce* implementations, and whose values must be known by the decoder: _time_width_p_, _cache_size_p_, _brpred_size_p_, and _f0s_width_p_.  In order to provide these values in a standard manner, the E-trace Standard Support Packet fast-track extension is required by Smstrce*/Ssstrce*.  This extension requires support for a standard Support packet (format 3 subformat 3, see chapter 7.5 of the Efficient Trace for RISC-V v2.0 Specification), which reports parameter values.
...
The TMB is configured using `sstrcambcs`, `sstrcambaddr`, `menvcfg`.STRCS, and `henvcfg`.STRCV.  See <<strccsrs>> and <<ssamb>> for details.
...
| trTeControl.trTeDisSelfHosted | 0 | An external debugger may set this bit to claim trace for itself, which will hide the presence of self-hosted trace from in-target software.
...
| trTeControl.trTeFormat | 0 or 1 | Implicit in extension definition: 0 for E-trace, 1 for N-trace
| trTeImpl.trTeVerMajor | 1 |
| trTeImpl.trTeVerMinor | 0 |
| trTeImpl.trTeCompType | 1 |
| trTeImpl.trTeProtocolMajor | 0 or 1 | Implicit in extension definition: 0 for E-trace v2.0, 1 for N-trace v1.0
...
| trTeInstFeatures.trTeInstImplicitReturnMode| 3 | Implicit return mode must have guaranteed correctness.  If implicit return mode is not supported then `mstrcctl`.EIR must to hardwired to 0.
...
| trTeFilter0MatchInst.trTeFilterMatchChoicePrivilege | {0, `vsstrcctl`.VS, `vsstrcctl`.VU, 0, `mstrcctl`.M, 0, `mstrcctl`.S, `mstrcctl`.U} | Matches the _priv_ encoding defined in Table 8 of the Efficient Trace for RISC-V Specification v2.0.
...
This section describes likely self-hosted trace configurations for select scenarios in a typical "rich OS" environment.  Note that this list is far from comprehensive, and there are scenarios where many more combinations of the bits below may be appropriate.

.Self-hosted Trace Configuration Scenarios
[options="header"]
|===
| Tracing | Application | OS | Application(s) + OS | Guest | Guest (emulated) | Guest(s) + Hypervisor
| `mstateen0`.STRC | 1 | 1 | 1 | 1 | 1 | 1
| `hstateen0`.STRC | NA | NA | NA | 1 | 0 | 0
| `menvcfg`.STRCS | 1 | 1 | 1 | 1 | 1 | 1
| `menvcfg`.STRCV | 0 | 0 | 0 | 1 | 1 | 0
| `mideleg`.LAMBI | 1 | 1 | 1 | 1 | 1 | 1
| `hideleg`.LAMBI | NA | NA | NA | 1 | 1 | 0
| `mstrcctl`.M | 0 | 0 | 0 | 0 | 0 | 0
| `mstrcctl`.S | 0 | 1 | 1 | 0 | 0 | 1
| `mstrcctl`.U | 0 | 0 | 1 | 0 | 0 | 1
| `vsstrcctl`.S | NA | NA | NA | 1 | 1 | 1
| `vsstrcctl`.U | NA | NA | NA | 1 | 1 | 1
|===

The "Application", "OS", and "Application(s) + OS" scenarios assume a bare-metal (V=0) configuration where only a user-mode application, the operating system, or some combination of the two, respectively, are traced.  The remaining scenarios assume a virtualized environment.  The "Guest" column assumes that a virtualization guest is accessing self-hosted trace directly.  The "Guest (emulated)" column assumes that the hypervisor is emulating guest access to self-hosted trace state, either via trap-and-emulate or via hypercall.  And finally, the "Guest(s) + Hypervisor" column assumes a hypervisor that is tracing both itself and one or more guests.

All selected scenarios involve delegation of self-hosted trace (`mstateen0`.STRC) and LAMBI interrupts (`mideleg`.LAMBI) to S-mode, to allow the kernel/hypervisor control of the capability.  Only in scenarios where a guest is accessing self-hosted trace state directly is self-hosted trace further delegated to VS-mode (`hstateen0`.STRC).  And only in scenarios where the effective privilege mode of trace writes is VS-mode are LAMBIs further delegated to VS-mode (`hideleg`.LAMBI), to allow the guest to service VS-stage translation faults and buffer management interrupts.

When tracing only while V=0, S-mode is chosen as the effective privilege mode of trace writes (`menvcfg`.STRCS and `henvcfg`.STRCV).  This allows the AMB to reside in kernel address space.  See <<ambaddr>> for configuration details.  When a virtualization guest is tracing ("Guest" column), or a hypervisor is emulating self-hosted trace on behalf of a guest ("Guest (emulated)" column) but allowing the guest to manage the AMB, VS-mode is chosen as the effective privilege mode of trace writes.  This allows the AMB to reside in guest address space.  And finally, when a hypervisor is tracing itself + guest(s) ("Guest(s) + Hypervisor" column), S-mode is chosen as the effective privilege mode of trace writes, allowing the AMB to reside in hypervisor address space.

NOTE: _A hypervisor might prefer a different configuration for the "Guest (emulated)" scenario.  One alternative would be for the AMB to be managed entirely by the hypervisor, and hence reside in hypervisor address space.  Such a configuration would choose S-mode as the effective privilege mode for trace writes, and would not further delegate LAMBIs to VS-mode._

The privilege mode filtering configurations chosen above simply reflect the modes in which each scenario chooses to trace.  There are many other configuration options within self-hosted trace, but they are orthogonal to these scenarios.
