...
|_call_counter_size_p_ | 0 |  Number of bits in the nested call counter is
2^*call_counter_size_p*^. Minimum number of entries is 2, so a value of
0 indicates that there is no implicit return call counter implemented.  Because the call
counter method can result in the decoder following a return to the wrong target, this
parameter is set to 0 for self-hosted trace.
